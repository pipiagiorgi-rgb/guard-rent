...
import { Check, Lock, Loader2, ArrowLeft, ArrowRight, CheckCircle2 } from 'lucide-react'
import Link from 'next/link'
import AssistantPanel from '@/components/ai/AssistantPanel'
import { WalkthroughVideoUpload } from '@/components/features/WalkthroughVideoUpload'
import { EvidencePhotoBox } from '@/components/features/EvidencePhotoBox'
...
export default function ShortStayPage() {
    ...
    const [arrivalPhotos, setArrivalPhotos] = useState<Asset[]>([])
    const [departurePhotos, setDeparturePhotos] = useState<Asset[]>([])
    const [arrivalVideo, setArrivalVideo] = useState<Asset | null>(null)
    const [departureVideo, setDepartureVideo] = useState<Asset | null>(null)
    ...
    async function loadData() {
        ...
        // Check if short_stay pack purchased via entitlements API (includes admin check)
        try {
            const entRes = await fetch(`/api/exports/status?caseId=${caseId}`)
            if (entRes.ok) {
                const status = await entRes.json()
                setHasPurchased(status.purchasedPacks?.includes('short_stay') || false)
        ...
        if (assets) {
            ...
            // Split by type and phase
            // Photos: checkin_photo = arrival, handover_photo = departure
            setArrivalPhotos(withUrls.filter(a => a.type === 'checkin_photo'))
            setDeparturePhotos(withUrls.filter(a => a.type === 'handover_photo'))

            // Videos: walkthrough_video with phase = 'check-in' or 'handover'
            setArrivalVideo(withUrls.find(a => a.type === 'walkthrough_video' && a.phase === 'check-in') || null)
            setDepartureVideo(withUrls.find(a => a.type === 'walkthrough_video' && a.phase === 'handover') || null)
        }
    ...
    async function handleSeal(phase: 'arrival' | 'departure') {
        // Admins and paid users can seal directly
        const canSeal = isAdmin || hasPurchased
        ...
        try {
            const endpoint = phase === 'arrival'
                ? '/api/short-stay/seal-arrival'
                : '/api/short-stay/seal-departure'
            ...
            if (!res.ok) {
                throw new Error(data.error || 'Failed to seal')
            }

            // Show success toast
            setToastMessage(data.message || (phase === 'arrival'
                ? 'Arrival evidence sealed — Photos are now timestamped and locked.'
                : 'Departure evidence sealed — Your short-stay evidence package is ready.'))
            setShowToast(true)
            ...
            setToastMessage(err.message || 'Failed to seal evidence')
            setShowToast(true)
            setTimeout(() => setShowToast(false), 5000)
        } finally {
            setSealing(false)
    ...
    const currentPhotos = activePhase === 'arrival' ? arrivalPhotos : departurePhotos
    const currentVideo = activePhase === 'arrival' ? arrivalVideo : departureVideo
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="flex gap-2 mb-6">
                ...
                >
                    ...
                    Arrival
                    {arrivalPhotos.length > 0 && <span className="text-xs opacity-75">({arrivalPhotos.length})</span>}
                </button>
                <button
                ...
                >
                    ...
                    Departure
                    {departurePhotos.length > 0 && <span className="text-xs opacity-75">({departurePhotos.length})</span>}
                </button>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">
                            {activePhase === 'arrival'
                                ? 'Document the condition of the property when you arrive.'
                                : 'Document the condition of the property before you leave.'}
                        </p>
                    </div>
                ...
                {/* Photos Section - Self-contained with embedded upload */}
                <div className="mb-6">
                    <EvidencePhotoBox
                        title={activePhase === 'arrival' ? 'Arrival photos' : 'Departure photos'}
                        description={
                            activePhase === 'arrival'
                                ? "Take photos of each room to document the property's condition at check-in."
                                : 'Capture the property condition before you leave to prove how it was returned.'
                        }
                        photos={currentPhotos}
                        onUpload={(e) => handlePhotoUpload(e, activePhase)}
                        onDelete={handleDeleteAsset}
                        isLocked={isCurrentPhaseComplete}
                        uploading={uploading}
                        recommendedCount={5}
                    />
                </div>
                ...
                <div className="mb-6">
                    <h3 className="text-sm font-medium text-slate-700 mb-2">
                        {activePhase === 'arrival' ? 'Arrival walkthrough video' : 'Departure walkthrough video'}
                        <span className="text-slate-400 font-normal"> (optional)</span>
                    </h3>
                    <p className="text-xs text-slate-500 mb-4">
                        {activePhase === 'arrival'
                            ? 'A short video capturing the condition of the property on arrival. Useful for disputes where photos alone are not enough.'
                            : 'A short video showing the property condition before departure.'}
                    </p>

                    <WalkthroughVideoUpload
                        caseId={caseId}
                        phase={activePhase === 'arrival' ? 'check-in' : 'handover'}
                        isLocked={isCurrentPhaseComplete}
                        isPaid={canSeal}
                        existingVideo={currentVideo ? {
                            assetId: currentVideo.asset_id,
                            fileName: currentVideo.storage_path.split('/').pop() || 'video',
                            durationSeconds: currentVideo.duration_seconds,
                            uploadedAt: currentVideo.created_at
                        } : undefined}
                ...
                {/* Actions - Seal button only (upload moved into photo box) */}
                {!isCurrentPhaseComplete ? (
                    <div className="pt-4 border-t border-slate-100">
                        <button
                            onClick={() => handleSeal(activePhase)}
                            disabled={currentPhotos.length === 0 || sealing}
                            className="w-full py-3 px-4 bg-slate-900 text-white rounded-xl font-medium hover:bg-slate-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                        >
                            {sealing ? (
                                ...
                                <>
                                    ...
                                    Seal {activePhase === 'arrival' ? 'Arrival' : 'Departure'}
                                </>
                            ) : (
                                <>
                                    <Lock className="w-5 h-5" />
                        ...
                        {currentPhotos.length === 0 && (
                            <p className="text-xs text-slate-400 text-center mt-2">
                                Add at least one photo to seal evidence
                            </p>
                        )}
                    </div>
                ) : (
            ...
            <AssistantPanel
                caseId={caseId}
                bookingContext={{
                    platform: caseData.platform_name || undefined,
                    checkIn: caseData.check_in_date || undefined,
                    checkOut: caseData.check_out_date || undefined,
                    arrivalPhotos: arrivalPhotos.length,
                    departurePhotos: departurePhotos.length
                }}
            />
        </div>
    )
